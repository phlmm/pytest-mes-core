...
from pytest_mes_core.host_adapters.hid_scanner import HeadlessBarcodeScanner, HidScannerTimeoutError
...
def test_wait_for_scan_kpenter_terminates(monkeypatch):
    ...
    monkeypatch.setattr(hid_scanner_mod, 'ecodes', MagicMock(EV_KEY=1))
    monkeypatch.setattr(hid_scanner_mod, 'categorize', _fake_categorize)
    scanner.device.read.return_value = [_FakeEvent('KEY_1'), _FakeEvent('KEY_2'), _FakeEvent('KEY_KPENTER')]
    monkeypatch.setattr(hid_scanner_mod.select, 'select', lambda *a, **kw: ([scanner.device.fd], [], []))
...
def test_enter_closes_non_matching_devices(monkeypatch):
    ...
    devices_by_path = {'/dev/input/event0': non_matching_1, '/dev/input/event1': non_matching_2, '/dev/input/event2': matching}
    monkeypatch.setattr(hid_scanner_mod, 'list_devices', lambda: list(devices_by_path.keys()))
    monkeypatch.setattr(hid_scanner_mod, 'InputDevice', lambda path: devices_by_path[path])
    scanner.__enter__()
