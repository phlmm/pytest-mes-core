...
def test_module_imports_and_resolve_device_path_globs(monkeypatch):
    ...
    monkeypatch.setattr(
        _glob, "glob", lambda pattern: ["/dev/usb-sd-mux/id-00048.00717"]
    )
...
def test_enter_releases_mutex_when_set_mux_state_fails():
    ...
    with patch(
        "pytest_mes_core.host_adapters.sd_mux.hardware_mutex",
        return_value=mutex_cm,
    ), patch.object(
        HostUsbSdMuxAdapter,
        "_set_mux_state",
        side_effect=HostAdapterError("device unplugged"),
    ):
