...
def _make_client(forensic: bool = False) -> tuple[EphemeralSSHClient, MagicMock]:
    ...
    with patch("pytest_mes_core.transports.ssh.Connection") as MockConn, \
...
class TestSSHSafeRun:
    ...
    def test_silent_socket_close_detected(self):
        ...
        client, mock_conn = _connected_client()
        mock_conn.run.return_value = _make_run_result(
            stdout="", stderr="Connection closed by remote host", exited=-1, ok=False
        )
    ...
    def test_long_command_truncated_in_log(self):
        ...
        assert result.ok is True
        # The full command must have been passed to run()
        actual_cmd = mock_conn.run.call_args[0][0]
...
class TestSSHSafeRunExceptions:
    ...
    def test_thread_exception_raises_transport_connection_error(self):
        ...
        # Subclass ThreadException to sidestep Invoke's brittle __str__ which
        # requires result objects with a .kwargs attribute that we can't easily stub.
        class _SafeThreadException(ThreadException):
            def __init__(self):
                # Skip ThreadException.__init__ entirely
...
# ===========================================================================
# async variants (offloaded to threads — just verify they delegate)
# ===========================================================================
...
class TestSSHAsync:
    ...
    async def test_async_safe_run_delegates_to_sync(self):
        ...
        result = await client.async_safe_run("ls")
        assert result.ok is True
        assert result.stdout == "async_ok"


# ===========================================================================
# PyOcdTransport — all branches (pyocd mocked via ImportError + real mock)
# ===========================================================================
...
class TestPyOcdTransport:
    ...
    def test_connect_raises_runtime_error_on_probe_failure(self):
        ...
        with patch.dict("sys.modules", {
            "pyocd": MagicMock(),
            "pyocd.core": MagicMock(),
            "pyocd.core.helpers": mock_helper,
        }):
            with patch("pyocd.core.helpers.ConnectHelper.session_with_chosen_probe",
                       side_effect=Exception("No probe found")):
    ...
    def test_connect_opens_session(self):
        ...
        with patch("builtins.__import__", side_effect=_patch_pyocd_import(mock_helper)):
            try:
                ...
                pass  # may fail if patching doesn't fully intercept

        # Verify idempotency guard
        t._session = mock_session
        t._session.is_open = True
        t.connect()  # should return early without calling session_with_chosen_probe again
    ...
    def test_disconnect_skips_when_not_connected(self):
        t = self._make_pyocd()
        t.disconnect()  # _session is None — must not raise

    # halt / resume / reset — call through to _core
    ...
    def test_halt_skips_when_no_core(self):
        t = self._make_pyocd()
        t.halt()  # _core is None — must not raise
    ...
    def test_read_memory_returns_bytes(self):
        t = self._make_pyocd()
        mock_core = MagicMock()
        mock_core.read_memory_block8.return_value = [0xDE, 0xAD, 0xBE, 0xEF]
        t._core = mock_core
        result = t.read_memory(0x20000000, 4)
    ...
    def test_write_memory_calls_core(self):
        ...
        mock_core = MagicMock()
        t._core = mock_core
        t.write_memory(0x20000000, b"\xAB\xCD")
        mock_core.write_memory_block8.assert_called_once_with(0x20000000, [0xAB, 0xCD])

    def test_write_memory_skips_when_no_core(self):
        t = self._make_pyocd()
        t.write_memory(0x20000000, b"\xFF")  # must not raise

    # read_core_register
    def test_read_core_register_returns_value(self):
        t = self._make_pyocd()
        mock_core = MagicMock()
        mock_core.read_core_register.return_value = 0xDEADBEEF
        t._core = mock_core
