...
def test_measure_throughput_success(eth_config):
    ...
    v_transport = MockTransport({'iperf3 -c': CommandResult(command='iperf', stdout=mock_iperf_json, stderr='', exited=0, ok=True, duration_s=5.0)})
...
def test_iperf3_json_parsing_success():
    mock_iperf_json = '{"end": {"sum_sent": {"bits_per_second": 945000000.0, "retransmits": 12}, "cpu_utilization_percent": {"host_total": 4.2}}}'
    v_transport = MockTransport({'iperf3 -c': CommandResult('iperf', stdout=mock_iperf_json, stderr='', exited=0, ok=True, duration_s=5.0)})
    ...
    assert result.metrics['throughput_mbps'] == 945.0
    assert result.context['retransmits'] == 12
    assert any(('iperf3 -c 192.168.100.1' in cmd for cmd in v_transport.command_history))