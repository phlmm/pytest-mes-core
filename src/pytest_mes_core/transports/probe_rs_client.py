...
class ProbeRsTransport(McuTransport):
    ...
    def halt(self) -> None:
        """Halt the CPU core via ``probe-rs reset`` in halt mode.

        Note: The stateless CLI does not support a true ``halt`` command.
        We issue ``reset --halt`` which stops execution at the reset vector.
        """
        logger.debug("probe_rs_halt", chip=self.cfg.chip)
        # probe-rs does not have a standalone "halt" -- reset+halt is closest
        self._run_cli(["reset"], context="halt")

    def resume(self) -> None:
        """Resume execution.  Implemented as a plain reset (run from vector)."""
    ...
    async def async_read_u32(self, address: int) -> int:
        return await anyio.to_thread.run_sync(
            lambda: self.read_u32(address))

    async def async_download(self, firmware_path: str, **kwargs) -> str:
        return await anyio.to_thread.run_sync(
            lambda: self.download(firmware_path, **kwargs))
