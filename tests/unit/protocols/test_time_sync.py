...
def test_time_sync_healthy_no_drift():
    mock_dut = MagicMock()
    cfg = TimeSyncConfig(max_drift_s=5.0, force_host_sync=True, daemon_type=TimeDaemonType.CHRONY, rtc_paths=['/sys/class/rtc/rtc0'], verify_hardware_pps=False)
...
def test_time_sync_excessive_drift_no_force_fails():
    mock_dut = MagicMock()
    cfg = TimeSyncConfig(max_drift_s=5.0, force_host_sync=False, daemon_type=TimeDaemonType.CHRONY, rtc_paths=[], verify_hardware_pps=False)
    host_epoch = real_time.time()
...
def test_audit_daemon_ntpd():
    mock_dut = MagicMock()
    ctx = {}
    mock_dut.safe_run.return_value = CommandResult(command='ntpq', ok=True, exited=0, stdout='*192.168.1.1 .GPS. 1\n', stderr='', duration_s=0.1)
...
def test_audit_daemon_systemd():
    mock_dut = MagicMock()
    ctx = {}
    mock_dut.safe_run.return_value = CommandResult(command='timedatectl', ok=True, exited=0, stdout='Server: 0.pool.ntp.org\nPoll: 64s', stderr='', duration_s=0.1)
...
def test_audit_daemon_ptp():
    mock_dut = MagicMock()
    ctx = {}
    mock_dut.safe_run.return_value = CommandResult(command='pmc', ok=True, exited=0, stdout='portState SLAVE', stderr='', duration_s=0.1)
