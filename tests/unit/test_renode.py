...
def test_start_redirects_output_to_log_file_not_pipe(tmp_path):
    ...
    with patch(
        "pytest_mes_core.host_adapters.renode.subprocess.Popen",
        return_value=fake_process,
    ) as mock_popen, patch.object(
        RenodeRunner, "_wait_for_port", return_value=True
    ):
        runner.start()
...
def test_start_failure_includes_log_path_in_error(tmp_path):
    ...
    with patch(
        "pytest_mes_core.host_adapters.renode.subprocess.Popen",
        return_value=fake_process,
    ), patch.object(RenodeRunner, "_wait_for_port", return_value=False):
        with pytest.raises(RenodeRunnerError, match="mes_renode_39998.log"):
            runner.start()

    # stop() (called internally on failure) must have closed the log handle.
