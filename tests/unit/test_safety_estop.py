...
def test_enter_arms_via_gpiod_v2_and_exit_releases():
    ...
    with patch('pytest_mes_core.host_adapters.safety.gpiod.request_lines', return_value=request_mock) as mock_request_lines:
        watchdog.__enter__()
        try:
...
def test_monitor_triggers_sigint_on_estop_active():
    ...
    with patch('pytest_mes_core.host_adapters.safety.os.kill', side_effect=fake_kill) as mock_kill, patch('pytest_mes_core.host_adapters.safety.os._exit') as mock_exit, patch('pytest_mes_core.host_adapters.safety.time.sleep') as mock_sleep:
        watchdog._monitor()
    assert kill_calls
    import signal
