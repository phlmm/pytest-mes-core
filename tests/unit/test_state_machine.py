...
async def test_fsm_power_cycle_error_handling(monkeypatch):
    ...
    fsm = EmbeddedLinuxStateMachine(psu=psu_mock, serial=serial_mock, ssh=ssh_mock, cfg=mock_cfg)
...
async def test_fsm_panic_event_during_boot(monkeypatch):
    ...
    _pin_cfg_defaults(mock_cfg)
    fsm = EmbeddedLinuxStateMachine(psu=MagicMock(), serial=serial_mock, ssh=MagicMock(), cfg=mock_cfg)
...
async def test_fsm_os_userland_heartbeat_fail_escalates_to_hot_login(monkeypatch):
    ...
    def mock_open_success(*args, **kwargs):
        ...
        import time
        yield PromptDetected(elapsed_s=time.time(), prompt_type='shell')
    stream_mock.open.side_effect = [mock_open_success()]
    mock_cfg = MagicMock()
    mock_cfg.cold_boot_timeout_s = 1.0
    _pin_cfg_defaults(mock_cfg)
    psu_mock = MagicMock()
    ...
    ssh_mock.ip_address = '127.0.0.1'
    ssh_mock.cfg.port = 2222
    ssh_mock.safe_run = MagicMock(side_effect=TransportConnectionError('dead'))
    fsm = EmbeddedLinuxStateMachine(psu=psu_mock, serial=serial_mock, ssh=ssh_mock, cfg=mock_cfg)
    fsm.event_stream = stream_mock
    fsm.machine.set_state(DutState.OS_USERLAND)
    fsm._do_power_off = MagicMock()
    fsm._do_energize = MagicMock()
    fsm.finalize_os_boot = MagicMock()
    fsm.boot_to_os()
    assert stream_mock.open.call_count == 1
    fsm.finalize_os_boot.assert_called_once()
    assert fsm.state == DutState.OS_USERLAND
...
async def test_fsm_verbose_logging_prints_at_info(monkeypatch):
    ...
    mock_cfg.cold_boot_timeout_s = 60.0
    _pin_cfg_defaults(mock_cfg)
    fsm_quiet = EmbeddedLinuxStateMachine(psu=MagicMock(), serial=serial_mock, ssh=MagicMock(), cfg=mock_cfg, verbose=False)
    fsm_quiet.event_stream = stream_mock
    fsm_quiet.event_wait_for_bootloader(intercept_autoboot=True)
    mock_logger.debug.assert_any_call('uart_rx', data='Loading Linux kernel...')
    mock_logger.reset_mock()
    stream_mock.open = mock_open
    fsm_verbose = EmbeddedLinuxStateMachine(psu=MagicMock(), serial=serial_mock, ssh=MagicMock(), cfg=mock_cfg, verbose=True)
    fsm_verbose.event_stream = stream_mock
    fsm_verbose.event_wait_for_bootloader(intercept_autoboot=True)
...
async def test_fsm_warm_reboot_uses_trap_when_autoboot_disabled(monkeypatch):
    ...
    _pin_cfg_defaults(mock_cfg)
    fsm = EmbeddedLinuxStateMachine(psu=MagicMock(), serial=serial_mock, ssh=ssh_mock, cfg=mock_cfg)
    fsm.machine.set_state(DutState.OS_USERLAND)
    fsm._set_uboot_trap_and_reboot = MagicMock()
    fsm.event_wait_for_bootloader = MagicMock()
    from transitions.core import EventData
    mock_event = MagicMock(spec=EventData)
    mock_event.kwargs = {}
    fsm._hw_boot_to_bootloader(mock_event)
    fsm._set_uboot_trap_and_reboot.assert_called_once()
    serial_mock.safe_run.assert_not_called()
    fsm.event_wait_for_bootloader.assert_called_once_with(intercept_autoboot=False)
