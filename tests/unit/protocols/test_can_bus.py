...
def test_configure_dut_interface_success(mock_dut):
    ...
    # Verify calls
    calls = [
        call('ip link set can0 down'),
        call('ip link set can0 type can bitrate 500000'),
        call('ip link set can0 up'),
        call('ip link show can0')
    ]
...
def test_fire_and_reap_dut_to_host(mock_dut, host_adapters):
    ...
    # Mock host adapter to successfully read the expected frame
    host_adapters["pcan0"].expect.return_value = True
    
    # TX: dut:can0, RX: host:pcan0
...
def test_fire_and_reap_host_to_dut(mock_dut, host_adapters):
    ...
    # Verify the daemon was killed and started
...
def test_validate_topology(mock_dut, host_adapters):
    ...
    # Mock fire_and_reap to always return True
    ...
    # Assert _fire_and_reap was called for each topology route
