...
def test_swupdate_verify_partition_flip_success():
    ...
    # After boot_to_os is called, the context should update. 
    # We can mock this by creating a side effect on boot_to_os that changes active_rootfs
...
def test_swupdate_verify_partition_flip_revert():
    ...
    def boot_to_os_side_effect():
        # U-Boot rejected the update and fell back to the original partition!
        mock_fsm.context.active_rootfs = "/dev/mmcblk0p2"
