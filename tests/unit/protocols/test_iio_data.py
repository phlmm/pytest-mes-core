...
def test_adc_measure_voltage_success():
    ...
    assert res.passed
    # 2048 * 0.805664 / 1000 = 1.6500 V
    assert abs(res.metrics["voltage_v"] - 1.6500) < 0.01

def test_adc_sensor_not_found():
    mock_dut = MagicMock()
    mock_dut.safe_run.return_value = CommandResult(
        command="grep", ok=False, exited=1, stdout="", stderr="", duration_s=0.1
    )
...
def test_adc_voltage_out_of_bounds():
    ...
    def safe_run_side_effect(cmd, *args, **kwargs):
        if "grep -l" in cmd:
            ...
            # Raw = 4095 => 3.3V, but max_v is 2.0 => FAIL
...
def test_adc_sample_count_mismatch():
    ...
    def safe_run_side_effect(cmd, *args, **kwargs):
        if "grep -l" in cmd:
            ...
            # Only 5 samples returned instead of 10
