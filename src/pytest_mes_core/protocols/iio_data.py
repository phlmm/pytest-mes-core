    ...
    def measure_voltage(
        ...
        try:
            ...
            if not res_resolve.ok or not res_resolve.stdout:
                return ValidatorResult(passed=False, error_msg=f"Sensor '{iio_device_name}' not found in sysfs.")

            base_path = res_resolve.stdout.strip().split('\n')[0].replace("/name", "")
            context_data["resolved_path"] = base_path

            logger.debug(f"[ADC {iio_device_name}:{channel}] Resolved to {base_path}. Fetching scale...")

            # 2. Scale Resolution
            res_scale = dut.safe_run(f"cat {base_path}/in_voltage{channel}_scale 2>/dev/null", timeout_s=2.0)
            if not res_scale.ok:
                return ValidatorResult(passed=False, error_msg=f"ADC Scale missing at {base_path}.", context=context_data)

            scale = float(res_scale.stdout.strip())
            logger.debug(f"[ADC {iio_device_name}:{channel}] Scale = {scale} mV/bit. Bursting {samples} samples...")
            ...
            # This guarantees the `delay_s` is perfectly accurate without SSH overhead.
            bash_loop = (
            ...
            if len(raw_strings) != samples:
                return ValidatorResult(passed=False, error_msg=f"Burst mismatch: Requested {samples}, got {len(raw_strings)}", context=context_data)
            ...
            if not passed:
                logger.warning(f"[ADC {iio_device_name}:{channel}] Out of bounds! {voltage_v}V not in [{min_v}V, {max_v}V]")
            else:
                logger.info(f"[ADC {iio_device_name}:{channel}] Averaged {avg_raw:.1f} raw -> {voltage_v} V")
...
class IioDacActuator:
    ...
    def set_voltage(
        ...
        try:
            ...
            if not res_resolve.ok or not res_resolve.stdout:
                return ValidatorResult(passed=False, error_msg=f"DAC '{iio_device_name}' not found.")
            ...
            if not res_scale.ok:
                return ValidatorResult(passed=False, error_msg=f"DAC Scale missing: {base_path}", context=context_data)
            ...
            if not res_write.ok:
                logger.error(f"[DAC {iio_device_name}:{channel}] Silicon rejected write operation: {res_write.stderr}")
