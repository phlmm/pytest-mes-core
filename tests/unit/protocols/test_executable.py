...
def test_run_binary_not_found():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="test", ok=False, exited=1, stdout="", stderr="", duration_s=0.1
    )
    mock_dut.is_connected = True
...
def test_fatal_signal_map_completeness():
    ...
    assert 132 in FATAL_SIGNALS  # SIGILL
    assert 134 in FATAL_SIGNALS  # SIGABRT
    assert 135 in FATAL_SIGNALS  # SIGBUS
    assert 136 in FATAL_SIGNALS  # SIGFPE
    assert 137 in FATAL_SIGNALS  # SIGKILL
    assert 139 in FATAL_SIGNALS  # SIGSEGV
