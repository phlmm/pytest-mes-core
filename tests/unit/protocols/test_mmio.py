...
def test_mmio_read_success_no_expected():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="devmem", ok=True, exited=0, stdout="0x12345678\n", stderr="", duration_s=0.05
    )
...
def test_mmio_read_with_mask():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="devmem", ok=True, exited=0, stdout="0xABCDEF01\n", stderr="", duration_s=0.05
    )
...
def test_mmio_read_expected_value_match():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="devmem", ok=True, exited=0, stdout="0xDEADBEEF\n", stderr="", duration_s=0.05
    )
...
def test_mmio_read_strict_devmem_blocked():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="devmem", ok=False, exited=1, stdout="", stderr="Operation not permitted", duration_s=0.05
    )
...
def test_mmio_write_success():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="devmem", ok=True, exited=0, stdout="", stderr="", duration_s=0.05
    )
