...
def test_edge_validator_button_press_success():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="gpiomon", ok=True, exited=0, stdout="1 rising-edge", stderr="", duration_s=1.2
    )
    ...
    mock_dut.safe_run.assert_called_once_with(
        "gpiomon --num-events=1 --rising-edge gpiochip0 5", timeout_s=10
    )
...
def test_edge_validator_irq_pulse():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="gpiomon", ok=True, exited=0, stdout="edge event", stderr="", duration_s=0.05
    )
...
def test_edge_validator_invalid_edge_type():
    mock_dut = MagicMock()
    # Bypass pydantic validation with a MagicMock config
    cfg = MagicMock()
...
def test_led_actuator_set_high():
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="gpioset", ok=True, exited=0, stdout="", stderr="", duration_s=0.1
    )

    GpioLedActuator.set_output(mock_dut, cfg, state=True)

    mock_dut.safe_run.assert_called_once_with(
        "gpioset --mode=wait gpiochip2 7=1 >/dev/null 2>&1 &", timeout_s=2.0
    )
...
def test_led_actuator_set_low():
    ...
    mock_dut.safe_run.assert_called_once_with(
        "gpioset --mode=wait gpiochip2 7=0 >/dev/null 2>&1 &", timeout_s=2.0
    )

def test_led_teardown_zero_leakage():
    mock_dut = MagicMock()
    mock_dut.safe_run.return_value = CommandResult(
        command="killall", ok=True, exited=0, stdout="", stderr="", duration_s=0.1
    )

    GpioLedActuator.teardown_zero_leakage(mock_dut)

    mock_dut.safe_run.assert_called_once_with(
        "killall -9 gpioset >/dev/null 2>&1 || true", timeout_s=3.0
    )
...
def test_loopback_mismatch_broken_trace(mock_sleep):
    ...
    def safe_run_side_effect(cmd, *args, **kwargs):
        if "gpioget" in cmd:
            # RX reads 0 when TX was driven to 1 = broken trace!
