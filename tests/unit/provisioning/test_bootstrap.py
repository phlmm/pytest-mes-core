...
def _make_cfg(**overrides):
    defaults = dict(
        reset_pin=5,
        boot_pins=[1, 2],
        boot_modes={"recovery": [1, 0], "normal": [0, 0]},
    )
...
def test_strobe_hardware_requests_v2_lines_and_strobes_correctly():
    ...
    with patch(
        "pytest_mes_core.provisioning.bootstrap.gpiod.request_lines",
        return_value=request_mock,
    ) as mock_request_lines:
        bootstrapper.set_boot_mode("recovery")

    # Requested the correct chardev path with all boot pins + reset pin.
    ...
    # boot pins asserted first (pin 1 -> state 1 -> ACTIVE, pin 2 -> state 0 -> INACTIVE),
    # then reset asserted (active-low => INACTIVE), then reset released (ACTIVE).
...
def test_strobe_hardware_reset_active_high_inverts_assert_release():
    ...
    calls = [call.args for call in request_mock.set_value.call_args_list]
    # boot pins both state 0 -> INACTIVE, then reset assert ACTIVE, release INACTIVE.
    assert calls == [
        (1, Value.INACTIVE),
        (2, Value.INACTIVE),
        (5, Value.ACTIVE),
