...
@pytest.fixture
def sysfs_cfg():
    return SysfsPollerConfig(
        polling_interval_s=1.0,
        targets={
            "cpu_temp": SysfsTargetConfig(path="/sys/class/thermal/thermal_zone0/temp", scale=0.001, unit="C"),
            "cpu_freq": SysfsTargetConfig(path="/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq", scale=0.001, unit="MHz"),
        }
    )
...
def test_parse_data_malformed_lines_skipped(sysfs_cfg):
    ...
    raw_lines = [
        "45000 || 1200000",
        "garbage line without separator",  # Should be skipped (wrong column count)
        "46000 || 1400000",
    ]
...
def test_parse_data_string_fallback(sysfs_cfg):
    ...
    # cpu_temp can't be float => string fallback
...
def test_context_manager_enter_serial_fallback(sysfs_cfg):
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="nohup", ok=True, exited=0, stdout="", stderr="", duration_s=0.1
    )
...
def test_context_manager_exit_serial_fetch(sysfs_cfg):
    ...
    # Should have metrics parsed from the fetched data
