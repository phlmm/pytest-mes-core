...
def test_verify_dependencies_all_present():
    mock_dut = MagicMock()
    # Empty stdout = all binaries found
    mock_dut.safe_run.return_value = CommandResult(
        command="for", ok=True, exited=0, stdout="", stderr="", duration_s=0.5
    )
...
def test_verify_dependencies_missing_binaries():
    mock_dut = MagicMock()
    # The bash loop echoes names of missing binaries
    mock_dut.safe_run.return_value = CommandResult(
        command="for", ok=True, exited=0, stdout="iperf3\ncansend\n", stderr="", duration_s=0.5
    )
...
def test_verify_dependencies_with_extras():
    ...
    assert res.passed
    # Verify the extra binary was included in the check
...
def test_system_health_high_cpu_load():
    ...
    def safe_run_side_effect(cmd, *args, **kwargs):
        if "dmesg" in cmd:
            ...
            # 2 cores * 1.5 = 3.0 threshold, load is 8.5 => thrashing
    ...
    # High load is a warning, not a failure
    assert res.passed
