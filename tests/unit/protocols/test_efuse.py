...
def test_efuse_read_success(efuse_cfg):
    ...
    mock_dut.safe_run.return_value = CommandResult(
        command="hexdump", ok=True, exited=0, stdout="DEADBEEF", stderr="", duration_s=0.1
    )
...
def test_efuse_read_wrong_length(efuse_cfg):
    mock_dut = MagicMock()
    # Hexdump returns 2 bytes instead of 4
    mock_dut.safe_run.return_value = CommandResult(
        command="hexdump", ok=True, exited=0, stdout="DEAD", stderr="", duration_s=0.1
    )
...
def test_efuse_read_failure(efuse_cfg):
    mock_dut = MagicMock()
    mock_dut.safe_run.return_value = CommandResult(
        command="hexdump", ok=False, exited=1, stdout="", stderr="Permission denied", duration_s=0.1
    )
...
def test_efuse_burn_idempotent_already_programmed(mock_sleep, efuse_cfg):
    ...
    # read_efuse returns the exact payload => already burned
    mock_dut.safe_run.return_value = CommandResult(
        command="hexdump", ok=True, exited=0, stdout="DEADBEEF", stderr="", duration_s=0.1
    )
...
def test_efuse_burn_dirty_region_rejected(mock_sleep, efuse_cfg):
    ...
    # Region is neither zero nor the target payload => previously burned with wrong data
    mock_dut.safe_run.return_value = CommandResult(
        command="hexdump", ok=True, exited=0, stdout="CAFEBABE", stderr="", duration_s=0.1
    )
    ...
    def safe_run_side_effect(cmd, *args, **kwargs):
        ...
        if "hexdump" in cmd:
            if call_count[0] == 1:
                ...
                # Readback is WRONG (silicon defect)
