...
import threading
import pytest
from unittest.mock import MagicMock, MagicMock, PropertyMock, patch
...
def _cmd_result(ok: bool=True, stdout: str='MES_PING') -> CommandResult:
    return CommandResult(command='echo MES_PING', stdout=stdout, stderr='', exited=0 if ok else 1, ok=ok, duration_s=0.01)
...
def test_disconnect_tears_down_both_transports_and_joins_thread():
    ...
    assert router._recovery_thread.is_alive()
    router.disconnect()
    assert not router._stop_recovery.is_set() or True
