...
async def test_async_wait_for_wakes_on_foreign_thread_transition():
    ...
    t.start()
    try:
        result = await fsm.async_wait_for("NETWORK_UP", timeout_s=5.0, poll_interval_s=0.01)
    finally:
        t.join(timeout=5.0)
...
async def test_async_wait_for_times_out_when_state_never_reached():
    fsm = McuAppStateMachine()
    result = await fsm.async_wait_for("APP_READY", timeout_s=0.2, poll_interval_s=0.02)
...
async def test_async_wait_for_returns_immediately_if_already_in_target_state():
    fsm = McuAppStateMachine()
    result = await fsm.async_wait_for("OFFLINE", timeout_s=1.0)
    assert result is True
...
async def test_state_durations_tracks_exited_and_current_state():
    ...
    # BOOTING was exited -> its accumulated duration must be a plausible,
    # small positive elapsed time (not a raw monotonic() timestamp).
    assert 0 < durations["BOOTING"] < 5.0

    # NETWORK_UP is the current state -> still reported, with a running
    # duration that keeps growing as time passes.
