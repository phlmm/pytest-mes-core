...
def test_pki_provision_credential_marks_only_chunk_writes_as_sensitive(tmp_path):
    ...
    local_key = tmp_path / "key.pem"
    # Long enough to be split into multiple base64 chunks (>64 chars per line).
    ...
    def safe_run_side_effect(cmd, *args, **kwargs):
        ...
        if "sha256sum" in cmd:
            sha_call_count += 1
            if sha_call_count == 1:
                # No cache hit on the idempotency check -- force the full transit path.
                return CommandResult(command=cmd, ok=False, exited=1, stdout="", stderr="", duration_s=0.1)
            # Final post-transit verification: report the correct hash.
    ...
    assert len(chunk_calls) > 1  # multiple base64 chunks
