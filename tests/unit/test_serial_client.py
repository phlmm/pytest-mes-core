...
class TestConnect:
    ...
    def test_connect_raises_on_busy_port_with_owner(self):
        ...
        with patch('serial.Serial', side_effect=busy_exc), patch('pytest_mes_core.host_adapters.diagnostics.ResourceDiagnostics.get_device_owner', return_value='minicom[PID=1234]'):
    ...
    def test_connect_raises_on_busy_port_without_owner(self):
        ...
        import serial
        busy_exc = serial.SerialException('device or resource busy')
        with patch('serial.Serial', side_effect=busy_exc), patch('pytest_mes_core.host_adapters.diagnostics.ResourceDiagnostics.get_device_owner', return_value=None):
...
class TestUsbHardwareReset:
    ...
    def test_resolve_usb_device_path_success(self, mock_open, mock_exists, mock_realpath):
        ...
        mock_realpath.side_effect = lambda path: {'/dev/serial/by-id/usb-some-serial-if04': '/dev/ttyACM5', '/sys/class/tty/ttyACM5/device': '/sys/devices/pci0000:00/0000:00:14.0/usb3/3-5/3-5.4/3-5.4:1.4'}.get(path, path)

        def exists_side_effect(path):
            if path == '/sys/class/tty/ttyACM5/device':
                return True
            if path in ('/sys/devices/pci0000:00/0000:00:14.0/usb3/3-5/3-5.4/busnum', '/sys/devices/pci0000:00/0000:00:14.0/usb3/3-5/3-5.4/devnum'):
                return True
            return False
